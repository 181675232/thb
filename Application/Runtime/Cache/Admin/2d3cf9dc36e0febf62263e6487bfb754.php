...
<div class="location">
  ...
  <i class="arrow"></i>
  <a href="/Admin/Goods"><span>商品列表</span></a>
  <i class="arrow"></i>
  <span>新增信息</span>
</div>
...
<div class="tab-content">
		...
		<dd>
			<div class="rule-single-select">
				<select id="ddlParentId" name="pid">
					...
					<?php if(is_array($type1)): $i = 0; $__LIST__ = $type1;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><option value="<?php echo ($val["id"]); ?>"><?php echo ($val["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
			</div>
		</dd>
	</dl>
    ...
    <dd>
        ...
        <input type="text" name="starttime" id="txtAddTime" Class="input date" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" datatype="/^\s*$|^\d{4}\-\d{1,2}\-\d{1,2}\s{1}(\d{1,2}:){2}\d{1,2}$/" errormsg="请选择正确的日期" sucmsg=" " />
        <i>日期</i>
      </div>
	</dd>
  </dl>
    ...
    <dd>
    	<div style="width: 190px" class="input-date">
        <input type="text" name="stoptime" id="txtAddTime" Class="input date" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" datatype="/^\s*$|^\d{4}\-\d{1,2}\-\d{1,2}\s{1}(\d{1,2}:){2}\d{1,2}$/" errormsg="请选择正确的日期" sucmsg=" " />
        <i>日期</i>
      </div>
	</dd>
  </dl>
    ...
    <dd>
		<div class="rule-multi-radio multi-radio">	   		
			<label><input type="radio" onclick="otable(this.value)" Value="2" name="istab" />是</label>
			<label><input type="radio" onclick="otable(this.value)" Value="1" checked="checked" name="istab" />否</label>
    	</div>
	</dd>
  </dl>
	<dl id="otable" style="display: none;">
		<dt>表格详情</dt>
		<dd>
			...
			<table id="tab" border = '1' bordercolor = '#ccc'>
				<tr><td style="padding: 5px;"><input type="text" name="tab[]" style="width: 160px;height: 26px;text-align: center;"  /></td><td style="padding: 5px;"><input type="text" name="tab[]" style="width: 160px;height: 26px;text-align: center;"  /></td><td style="padding: 5px;"><input type="text" name="tab[]" style="width: 160px;height: 26px;text-align: center;"  /></td></tr>
			</table>
		</dd>
	</dl>
